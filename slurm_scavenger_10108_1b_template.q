...
#SBATCH --job-name=sens_1b_xfactor_number_low_O
#SBATCH --output=errors/xfactor_number_1b._%A_%a.err
#SBATCH --ntasks=1
...
#SBATCH -e errors/xfactor_number_1b._%A_%a.err
...
export MESA_INLIST=$MESA_BASE/inlist_main_1b_low_O
export MESA_RUN=/work/al363/runs/low_overhead/runs_xfactor_number

#mkdir $MESA_BASE/runs_x100_number
echo -e "the mesa run folder is $MESA_RUN\n" >> /hpc/group/physics/al363/sens/errors/xfactor_number_1b._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
shopt -s nullglob
...
cd $MESA_RUN/${index2}
cp $MESA_BASE/inlist_cluster_abund_templatefactor  ./inlist_cluster_low_overhead_1b
rxn1=$(sed -n ''${index2}'p' $MESA_BASE/reaction_list_305_10108.txt)
sed -i 's|reaction_name1|'$rxn1'|g'  inlist_cluster_low_overhead_1b
rxn2=$(sed -n ''${index1}'p' $MESA_BASE/reaction_list_305_10108.txt)
sed -i 's|reaction_name2|'$rxn2'|g'  inlist_cluster_low_overhead_1b
max_model=$(sed -n ''${index3}'p' $MESA_BASE/1b/max_model_xfactor_number.txt)
sed -i 's|max_numb|'${max_model}'|g'  inlist_cluster_low_overhead_1b
...
if [ "${model}" = "0" ]; then
    ...
    $MESA_BASE/star >> /hpc/group/physics/al363/sens/errors/xfactor_number_1b._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
    date "+DATE: %Y-%m-%d%nTIME: %H:%M:%S"
	...
	cd ../
	 date "+DATE: %Y-%m-%d%nTIME: %H:%M:%S" 
        ...
        echo "DNC" >> /hpc/group/physics/al363/sens/errors/xfactor_number_1b._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
