...
#SBATCH --job-name=sens_x100_10_low_O
#SBATCH --output=errors/x100_10._%A_%a.err
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --mem-per-cpu=3072
#SBATCH --array=3,95,105,197,207,271,337,346,463,467,540,582,631,669,738,776,825,831,910,980,1016,1036,1152,1170,1220,1241,1310,1358,1420
#SBATCH --mail-type=BEGIN,END
#SBATCH --mail-user=amberlauer@gmail.com
#SBATCH -e errors/x100_10._%A_%a.err
...
export MESA_RUN=/work/al363/runs/low_overhead/runs_x100_verify


#mkdir $MESA_BASE/runs_x100_1
cd $MESA_RUN

echo -e "the mesa run folder is $MESA_RUN\n" >> /hpc/group/physics/al363/sens/errors/x100_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err
...
if $empty; then
    ...
    cd $MESA_RUN/${index2}
    cat $MESA_BASE/inlist_cluster_template100 > ./inlist_cluster_low_overhead
    ...
    $MESA_BASE/star >> /hpc/group/physics/al363/sens/errors/x100_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err
else
    cd $MESA_RUN/${index2}
    if [! ls ./final_profile* 1> /dev/null 2>&1]; then
    	echo "this_model_is_finished"  >> /hpc/group/physics/al363/sens/errors/x100_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
    else
    	echo "starting from photo" >> /hpc/group/physics/al363/sens/errors/x100_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
    	cd ./photos
