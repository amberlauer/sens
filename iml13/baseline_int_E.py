...
for i in range(1): # goes through odd files
    ...
    for j in range(num_models):
        ...
        log_total_E.append(all_models[j][column_name['log_tot_E']])
        total_E.append(all_models[j][column_name['tot_E']])
...
csv_name = csv_path + 'baseline_Int_E' + '.csv'
...
with open(csv_name, 'w', newline='') as data:
        ...
        writer.writerow(['Run Number', 'Integrated E', 'Difference from Baseline', 'Max E (avg func)'])
        writer.writerow([model_num[i],integrals[0][0],perc_diff[0],avg_max_lums[0]] )
