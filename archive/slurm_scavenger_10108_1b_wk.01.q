...
#SBATCH --job-name=sensitivity_10108_wk_.01
...
export MESA_RUN=/work/al363/runs/runs_x.01_wk_both
...
#max_model=$(sed -n ''${index3}'p' $MESA_BASE/1b/max_model_x.01_wk.txt)
#sed -i 's|max_numb|'${max_model}'|g'  inlist_cluster

#model=$(sed -n ''${index3}'p' $MESA_BASE/1b/restart_model_x.01_wk.txt)
model=0
