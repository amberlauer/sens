...
#SBATCH --job-name=sens_x.01_10_low_O
#SBATCH --output=errors/x.01_10._%A_%a.err
#SBATCH --ntasks=1
...
#SBATCH -e errors/x.01_10._%A_%a.err
...
export MESA_RUN=/work/al363/runs/low_overhead/runs_x.01_verify


#mkdir $MESA_BASE/runs_x100_1
cd $MESA_RUN

echo -e "the mesa run folder is $MESA_RUN\n" >> /hpc/group/physics/al363/sens/errors/x.01_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err
...
if $empty; then
    ...
    $MESA_BASE/star >> /hpc/group/physics/al363/sens/errors/x.01_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err
else
    cd $MESA_RUN/${index2}
    if [! ls ./final_profile* 1> /dev/null 2>&1]; then
    	echo "this_model_is_finished"  >> /hpc/group/physics/al363/sens/errors/x.01_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
    else
    	echo "starting from photo" >> /hpc/group/physics/al363/sens/errors/x.01_10._${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err 
    	cd ./photos
